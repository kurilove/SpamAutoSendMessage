...
def get_proxy(proxy_num):
    with open("E:/pythonProject/SpamAutoSendMessage/Data/proxy", "r", encoding="utf-8") as proxy_file:
...
def set_username_for_first_mes(user):
    with open('E:/pythonProject/SpamAutoSendMessage/Data/FirstMessageSend.txt', "a", encoding="utf-8") as file:
        try:
            file.write(f'{user}\n')
...
def set_username_for_second_mes(user):
    with open('E:/pythonProject/SpamAutoSendMessage/Data/SecondMessageSend.txt', "a", encoding="utf-8") as file:
        try:
            file.write(f'{user}\n')
        except Exception as ex:
            print(ex)
...
def get_use_username_data_for_first_mes() -> list:
    username_data = []
    with open("E:/pythonProject/SpamAutoSendMessage/Data/FirstMessageSend.txt", "r", encoding="utf-8") as file:
...
def get_use_username_data_for_second_mes() -> list:
    username_data = []
    with open("E:/pythonProject/SpamAutoSendMessage/Data/SecondMessageSend.txt", "r", encoding="utf-8") as file:
        for username in file.read().split("\n"):
            username_data.append(username)
